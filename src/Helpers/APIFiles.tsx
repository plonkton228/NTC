...
export class APIFiles {
   ...
   async getFiles(){
      const response = await axios.get<FileType[]>(process.env.SERVE_URL+'/files');
   ...
   async getFilesByName(id : string | undefined){
    try {
        const response = await axios.get<FileTypeRequest>(process.env.SERVE_URL+`/files/${id}`);
        return response.data
    } catch (error) {
        alert(error)
    }
   ...
   async getFilesById(id : string | undefined){
    try {
        const response = await axios.get<FileTypeRequest>(process.env.SERVE_URL+`/files/${id}/info`);
        return response.data
    } catch (error) {
        alert(error)
    }
