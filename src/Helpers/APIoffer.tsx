...
export class APIOffer {
    ...
    async createOffer(offer : OfferCreateType){
     ...
     async getOfferById  (id : string | undefined)  {
      try {
        const response = await  axios.get<OfferUpdateType>(process.env.REACT_APP_API_URL+`/assistance/${id}`)
        return response
      } catch (error) {
    ...
    async getSubOffer(id: string | undefined) {
      try {
        const data =  await axios.get<OfferChildCreateType>(process.env.REACT_APP_API_URL+`/assistance/subassistance/${id}`);
        return data.data;
      } catch (error) {
        alert(error)
      }
